...
const UsuarioList = () => {
  ...
  function update() {
    if (selectedCustomers) {
      setUser(selectedCustomers)
      navigate("/usuario", selectedCustomers);
    }
  }
  ...
  const deletar = () => {
    if (selectedCustomers) {
      confirmDialog({
        message: `Você confirma deletar o ID: ${selectedCustomers.user_id} - ${selectedCustomers.nome} ?`,
        header: 'Confirmar Exclusão!',
        icon: 'pi pi-info-circle',
        acceptClassName: 'p-button-danger',
        accept: () => deletarUsuario(),
  ...
  async function deletarUsuario() {
    await api
      .delete(`/usuario/${selectedCustomers.user_id}`)
      .then((response) => response.data)
      .then(() => loadDados())
      .then(toast.success("Deletado com sucesso!"))
      .catch((err) => {
        toast.error("ERRO: usuário com carga Patrimonial e não pode ser deletado: " + err)
      });
  }
  ...
  return (
    <div className={styles.container}>
      ...
      <DataTable className={styles.lista} value={customers}
        ...
        <Column className={styles.colunaFull} field="setor.nome" header="Setor"></Column>
